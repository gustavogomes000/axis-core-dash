...
export const Route = createFileRoute("/_auth/emporio/contas-pagar")({ component: Page });

function Page() {
  const { data = [], isLoading } = useList<any>("contas_pagar", "cp-emp", "*", { column: "data_vencimento" });
  const upsert = useUpsert("contas_pagar", ["cp-emp"]);
  const del = useDelete("contas_pagar", ["cp-emp"]);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setOpen(false); setF({ descricao: "", valor: 0, data_vencimento: "", categoria: "outros" });
  };

  const filtradas = useMemo(() => {
    const q = busca.trim().toLowerCase();
    if (!q) return data;
    return data.filter((c: any) => (c.descricao ?? "").toLowerCase().includes(q) || (c.categoria ?? "").toLowerCase().includes(q));
  }, [data, busca]);
  ...
  return (
    <div>
      ...
      <div className="bg-card rounded-lg border">
        <Table>
          <TableHeader><TableRow><TableHead>Descrição</TableHead><TableHead>Categoria</TableHead><TableHead>Vencimento</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Valor</TableHead><TableHead /></TableRow></TableHeader>
          <TableBody>
            {isLoading && <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground">Carregando…</TableCell></TableRow>}
            {!isLoading && filtradas.length === 0 && <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-10">Nenhuma conta cadastrada. Clique em "Nova" para começar.</TableCell></TableRow>}
            {filtradas.map((c: any) => (
              <TableRow key={c.id}>
